...
function parseData(rows) {
  return rows.map(r => {
    ...
    if (dept) {
      ...
      region = dept;
      if (dept === 'Lima City') {
        city = String(r['Where in Lima Downtown'] || '').trim();
      ...
      region = String(r['Province'] || '').trim();
      city   = String(r['City_col_AR'] || '').trim();
    } else if (country === 'Guyana' || country === 'Paraguay') {
      region = String(r['City_col_AR'] || '').trim();
      city   = String(r['Region'] || '').trim();
    } else {
      // Generic fallback for any future countries
      region = String(r['Department/Region/State'] || r['Region'] || r['Province'] || '').trim();
      city   = String(r['City'] || r['City_col_AR'] || '').trim();
    }
...
async function aggregateByRestaurant(data) {
  ...
  data.forEach(item => {
    ...
    if (item.customPrice > 0) {
      ...
      if (item.customCurrency) g.customCurrencies.push(item.customCurrency);
    }
...
function selFill(id, set) {
  let arr = [...set].filter(Boolean).sort();
  arr.unshift('All');
...
function setupEventListeners() {
  ...
  document.getElementById('countryFilter').onchange = () => { populateRegion(); filterData(); };
  document.getElementById('regionFilter').onchange = () => { populateCity(); filterData(); };
